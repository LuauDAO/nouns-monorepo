...
task('set-merkle', 'Sets merkle tree and reserve')
  ...
  .setAction(async ({ merkleQuantity, nounsToken }, { ethers }) => {
    if (typeof merkleQuantity != 'undefined' && merkleQuantity < recipients.length)
      throw `Invalid merkle quantity (${merkleQuantity}) is less than merkle recipients (${recipients.length})`;
    ...
    recipients.map(account => {
      if (!ethers.utils.isAddress(account)) throw `Invalid account: ${account}`;
    });

    const merkleTree = new MerkleTree(
      recipients.map(account => hashAccount(account)),
      keccack256,
      { sortPairs: true },
    );

    const tx = await nounsTokenContract.setRoot(
      merkleTree.getHexRoot(),
      merkleQuantity || recipients.length,
    );
